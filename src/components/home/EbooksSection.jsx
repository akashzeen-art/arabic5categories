  ...
  return (
    <section className="py-24 relative bg-[#0a1628]">
      ...
      <div className="container mx-auto px-4 md:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b-2 border-[#c9a84c]/20 pb-6">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <BookOpen className="text-[#c9a84c] w-8 h-8" />
        ...
        <div className="flex flex-nowrap overflow-x-auto pb-12 pt-8 snap-x snap-mandatory no-scrollbar gap-8 md:gap-12">
          {items.map((item, idx) => (
             <motion.div
                ...
                whileHover={{ y: -20 }}
                className="shrink-0 w-48 shadow-[15px_15px_30px_rgba(0,0,0,0.4)] snap-start relative group cursor-pointer"
                onClick={() => handleClick(item)}
             >
                <div className="absolute inset-y-0 left-0 w-2 bg-gradient-to-r from-white/40 to-transparent z-10"></div>
                <div className="absolute -right-3 top-2 bottom-2 w-3 bg-[#c9a84c]/30 rounded-r-md skew-y-[45deg] origin-left group-hover:bg-[#c9a84c]/50 transition-colors"></div>
                ...
                <div className="aspect-[2/3] relative rounded-l-sm rounded-r-md overflow-hidden border-2 border-[#1E293B] bg-slate-200">
                  ...
                  <div className="absolute inset-0 bg-slate-900/90 flex flex-col justify-center items-center p-4 text-center opacity-0 group-hover:opacity-100 transition-opacity">
                     ...
                     <span className="text-[#c9a84c] text-sm font-bold bg-white/10 px-3 py-1 rounded-full">{item.audioUrl ? 'Listen' : 'اقرأ'}</span>
                  </div>
                </div>
